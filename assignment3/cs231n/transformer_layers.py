...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        ############################################################################
        seq = torch.arange(0, max_len).unsqueeze(1)
        div_term = torch.exp(torch.arange(0, embed_dim, 2) * (-math.log(10000.0) / embed_dim))
        pe[:, :, 0::2] = torch.sin(seq * div_term)
        pe[:, :, 1::2] = torch.cos(seq * div_term)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        ############################################################################
        H = self.n_head
        # Project query/key/value and split the embedding dimension across heads:
        # (N, S, E) -> (N, S, H, E/H) -> (N, H, S, E/H)
        q = self.query(query).reshape(N, S, H, self.head_dim).transpose(1, 2)
        k = self.key(key).reshape(N, T, H, self.head_dim).transpose(1, 2)
        v = self.value(value).reshape(N, T, H, self.head_dim).transpose(1, 2)

        # Scaled dot-product attention scores, shape (N, H, S, T)
        scores = torch.matmul(q, k.transpose(-2, -1)) / math.sqrt(self.head_dim)

        # Prevent masked positions from influencing the output
