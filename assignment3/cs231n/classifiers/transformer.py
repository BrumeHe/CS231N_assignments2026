...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        # Project the image features into the same dimensions, and treat the
        # projected feature as a length-1 memory sequence for cross-attention
        image_features = self.visual_projection(features).unsqueeze(1)

        # Prepare the mask for masking out future timesteps
        tgt_mask = torch.tril(torch.ones(T, T, dtype=torch.bool, device=captions.device))

        # Apply the decoder layers
        decoder_output = self.transformer(caption_embeddings, image_features, tgt_mask=tgt_mask)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        x_pos_encoded = self.positional_encoding(x_embedded)
        encoded_output = self.transformer(x_pos_encoded)
        pooled_output = torch.mean(encoded_output, dim=1)
