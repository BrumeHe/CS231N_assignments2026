...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        for i in range(self.num_layers):
            self.params['W{}'.format(i + 1)] = np.random.normal(
                0, weight_scale, (layer_dims[i], layer_dims[i + 1])
            )
    ...
    def loss(self, X, y=None):
        ...
        scores, final_cache = affine_forward(
            out, self.params['W{}'.format(self.num_layers)], self.params['b{}'.format(self.num_layers)]
        )
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        dout, grads['W{}'.format(self.num_layers)], grads['b{}'.format(self.num_layers)] = affine_backward(
            dscores, final_cache
        )
        grads['W{}'.format(self.num_layers)] += self.reg * self.params['W{}'.format(self.num_layers)]

        for i in range(self.num_layers - 2, -1, -1):
            fc_cache, norm_cache, relu_cache, dropout_cache = caches[i]
